...
def main():
    ...
    n_fake_answers = st.number_input("Number of wrong answers", min_value=0, max_value=MAX_N_FAKE_ANSWERS, value=DEFAULT_N_FAKE_ANSWERS)

    if not all_in_state_and_not_none([VAR_RETRIEVED, VAR_ANSWER_LIST]):
        with st.spinner("Generating Question..."):
            retrieved_title, combined_synopsis = movie_suggestion.select_film_and_generate_synopsis()
    ...
    for i, an_answer in zip(button_labels, answer_list, strict=True):
        ...
        with letter_col:
            b_output = st.button(label=f"{i}", disabled=is_answered, on_click=is_is_answered)
